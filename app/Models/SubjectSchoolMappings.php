...
class SubjectSchoolMappings extends Model
{
    ...
    public function getClassNameById(){
        $subkey = 'subject_id';
        if($this->$subkey != ""){
            $data = ClassSubjectMapping::where(cn::CLASS_SUBJECT_MAPPING_SCHOOL_ID_COL,Auth()->user()->school_id)->where('subject_id',$this->$subkey)->select(cn::CLASS_SUBJECT_MAPPING_CLASS_ID_COL.' as name')->get()->toArray();
            if(isset($data) && !empty($data)){
                $data = array_column($data,'name');
                $dataTitle = Grades::whereIn('id',$data)->select(\DB::raw('GROUP_CONCAT('.cn::GRADES_NAME_COL.') as names'))->get()->toArray();
